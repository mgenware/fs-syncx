...
class FSSyncx {
  ...
  static listPathsCore(
    ...
    return FSSyncx.callWrapper(catchExp, path, null, () => {
      ...
      paths.forEach((value) => {
        // check dir or path glob first
        if (glob && globTarget !== GLOB_TARGET.files) {
          if (!FSSyncx.isGlobMatch(value, glob)) {
            return;
          }
        }
        const pathInfo = FSSyncx.pathInfo(nodepath.join(path, value), catchExp);
        if (pathInfo) {
          // check file or path glob
          if (glob && globTarget !== GLOB_TARGET.dirs) {
